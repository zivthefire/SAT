...
public class Vector2D {
    ...
    public void render(Graphics2D g2d, int startX, int startY, boolean drawArrowhead) {
     g2d.setColor(Color.white);

        // Adjust startY to reflect the inverted y-axis in the graphics context
        int invertedStartY = Math.abs(startY);


        // Draw the line starting from the specified point
        g2d.drawLine(startX, invertedStartY, startX + (int) x, invertedStartY + (int) y);

        // Draw arrowhead (optional)
        if (drawArrowhead) {
            drawArrowHead(g2d, startX + (int) x, invertedStartY + (int) y, Math.atan2(-y, x));
        }
    }


    // Helper method to draw an arrowhead
    private void drawArrowHead(Graphics2D g2d, int endX, int endY, double angleInRadians) {
        int arrowSize = 10; // Adjust the size of the arrowhead as needed
        double arrowAngle = Math.toRadians(30); // Adjust the angle of the arrowhead as needed

        // Calculate coordinates for arrowhead with adjusted angle
        int x1 = endX - (int) (arrowSize * Math.cos(angleInRadians - arrowAngle));
        int y1 = endY + (int) (arrowSize * Math.sin(angleInRadians - arrowAngle));
        int x2 = endX - (int) (arrowSize * Math.cos(angleInRadians + arrowAngle));
        int y2 = endY + (int) (arrowSize * Math.sin(angleInRadians + arrowAngle));
    ...
    public static Vector2D fromPoint(Point2D.Double p) {
